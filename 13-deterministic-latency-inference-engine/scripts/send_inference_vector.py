...
def send_inference_vector(interface, feature_id, feature_val, payload_bytes):
    print(f"Sending packets on {interface}...")
    
    # 1. Construct Payload
    #    Feature ID (4 bytes) + Feature Value (4 bytes, Big Endian) + Extra Payload
    feature_id_data = feature_id.ljust(4, b'\x00')
    feature_val_data  = struct.pack('>I', feature_val)
    
    # Total payload = 8 bytes + extra
    # DEBUG: Fill surround bytes with the Value as well to catch alignment errors
    # If we read one byte early or late, we still get the value.
    if payload_bytes == b'\x01\x02\x03\x04\x05\x06\x07\x08':
        payload_bytes = struct.pack('B', feature_val) * 16 # Repeat value 16 times
    
    # Pre-pad value to fill alignment gaps?
    # No, just send standard.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Send Inference Vector UDP Packet to FPGA')
    parser.add_argument('--iface', default=INTERFACE, help=f'Network Interface (default: {INTERFACE})')
    parser.add_argument('--id', default='0050', help='Feature ID (4 chars)')
    parser.add_argument('--val', type=int, default=100, help='Feature Value (Integer)')
    ...
    try:
        send_inference_vector(args.iface, args.id.encode(), args.val, payload)
    except Exception as e:
        print(f"\nError: {e}")
        print("You might need to run with sudo: sudo python3 scripts/send_inference_vector.py")
